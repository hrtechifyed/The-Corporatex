const base = (process.env.CORPORATEX_BASE_URL || 'https://hrtechifyed.github.io/The-Corporatex').replace(/\/$/, '');
const endpoints = ['/', '/stories.html', '/how-it-works.html', '/feedback.html'];
...
const readinessWindowMs = Number(process.env.CORPORATEX_READINESS_WINDOW_MS || 5 * 60 * 1000);
...
async function timedFetch(path, requestTimeoutMs = timeoutMs) {
  ...
  try {
    const response = await fetch(`${base}${path}`, { signal: controller.signal, headers: { 'user-agent': 'CorporateX-Production-Smoke/2.0' } });
...
async function waitForProductionReadiness() {
  ...
  while (Date.now() < deadline) {
    attempt += 1;
    last = await timedFetch('/', 30000);
    if (last.ok) {
      console.log(`GitHub Pages became ready after ${attempt} attempt${attempt === 1 ? '' : 's'} (${Math.round(last.duration)}ms).`);
      return;
    }
  ...
  throw new Error(`GitHub Pages did not become ready within ${Math.round(readinessWindowMs / 1000)} seconds. Last result: HTTP ${last?.status || 0} ${last?.error || ''}`);
}

await waitForProductionReadiness();
for (const path of endpoints) {
  const warm = await timedFetch(path, 30000);
