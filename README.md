...
### If the site appears as unstyled text

Do not open the repository files directly and do not point GitHub Pages at the source tree. Tailwind directives, React components, SVG scene composition, and application routes are compiled by Next.js; an unbuilt source deployment will therefore look like plain text or fail to load its styles. In Vercel, confirm **Framework Preset: Next.js**, **Build Command: `next build`**, and **Output Directory: Next.js default**. The included `vercel.json` pins framework detection. Locally, use `npm run dev` rather than opening a file with a `file://` URL. In browser developer tools, verify `/_next/static/css/...` returns HTTP 200. A 404 there means the deployment is serving source files instead of the Next.js build.
