...
const journeyRuntime = await readFile('src/journey-fourth-stage.js', 'utf8');
...
test('header signal uses four real independent workplace journey milestones', () => {
  ...
  assert.doesNotMatch(journey, /nth-child\(3\)::after/);
  assert.match(journey, /nth-child\(4\)/);
  assert.match(journey, /animation-name:\s*cx-exit-stop-four/);
  assert.match(journey, /@keyframes cx-exit-stop-four/);
  assert.match(journey, /83%, 91%/);
  assert.match(journeyRuntime, /milestones\.length >= 4/);
  assert.match(journeyRuntime, /index < 4/);
  assert.match(journeyRuntime, /signal\.append\(document\.createElement\('i'\)\)/);
  assert.match(build, /src\/journey-fourth-stage\.js/);
  assert.ok(build.indexOf('src/github-production.js') < build.indexOf('src/journey-fourth-stage.js'));
});

test('header signal owns a dedicated mobile strip without touching page components', () => {
  ...
  assert.match(journey, /nth-child\(4\) \{ left:\s*92%/);
