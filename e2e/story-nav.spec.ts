...
test('Story Beat Next or Skip control sits above the writing box', async ({ page }) => {
  await page.route('**/api/location/validate?**', async (route) => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ valid: true, matchedName: 'Bengaluru, Karnataka, India' }) }));
