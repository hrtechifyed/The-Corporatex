...
function accountPanel() {
  ...
  panel.innerHTML = `
    ...
    <div class="cx-submit-next"><strong>What happens next</strong><span>1. Your story goes into private moderation.</span><span>2. HRTechify receives a review notification.</span><span>3. You receive a CorporateX receipt by email.</span><span>4. If approved, it appears in Stories and we email you again.</span></div>`;
...
function showAccountPanel(payload) {
  ...
  panel.hidden = false;
  notify('Your story is saved in this browser. Add your private email and password below to submit it.', '', 12000);
...
async function send(payload) {
  ...
  window.setTimeout(() => location.replace(`account.html?submitted=1&tab=submissions&id=${encodeURIComponent(data?.id || payload.draftId)}`), 4800);
}
...
async function handleAccountSubmit(event) {
  ...
  try {
    if (accountMode === 'signin') {
      ...
      if (error) throw new Error('Email or password did not match. Try again, or create a new account if this is your first submission.');
    ...
    notify('Almost done. Check your email to confirm your CorporateX account. This is an account-confirmation email, not a sign-in link.', 'success', 14000);
...
if (prototypeNote) prototypeNote.textContent = 'After you submit, your story enters private moderation. Your email stays private. If approved, the story appears in Stories and CorporateX emails you again.';
