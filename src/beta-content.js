...
function showAwaitingContributors(status) {
  ...
  empty.append(create('p', 'eyebrow', 'CONTROLLED BETA'), heading, create('p', '', `The first release opens after contributor approval and human moderation. Target: ${status?.target_story_count || 15} stories across at least ${status?.minimum_employer_count || 5} employers.`));
  list.before(empty);
  const preview = create('details', 'demo-layout-preview');
  const body = create('div', 'demo-layout-body');
  list.replaceWith(preview);
  body.append(list);
  preview.append(create('summary', '', 'Preview the fictional directory layout'), body);
  empty.after(preview);
}
