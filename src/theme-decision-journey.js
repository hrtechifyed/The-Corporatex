...
// This enhancement replaces only the Free-flow “Choose a chapter—or skip it” section.
// Guided Story retains its separate turning-point and narrative-arc experience.
if (pageName === 'freeflow-story.html') {
...
function mountThemeDecisionJourney(section) {
  ...
  section.innerHTML = `
    <header class="theme-decision-heading">
      <div>
        <p class="eyebrow">OPTIONAL THEMES</p>
        <h2>Choose a chapter—or skip it.</h2>
      </div>
      <p>Reveal one card at a time. Keep what fits, ignore what does not, or return later.</p>
    </header>
  ...
  const emitState = (eventName = 'themejourneychange') => {
    const summary = summarizeThemeDecisions(state);
    form?.dispatchEvent(new CustomEvent(eventName, {
      bubbles: true,
      detail: {
        confirmed: state.confirmed,
        cards: state.cards.map(({ id, theme, status }) => ({ id, theme, status })),
        selected: summary.selected.map(({ id, theme }) => ({ id, theme })),
      },
    }));
  };

  const nextUnansweredIndex = () => {
    const isOpen = (card) => ['unseen', 'viewed', 'skipped'].includes(card.status);
    const after = state.cards.findIndex((card, index) => index > state.currentIndex && isOpen(card));
    if (after >= 0) return after;
    return state.cards.findIndex(isOpen);
  };

  const goTo = (index, options = {}) => {
    clearAdvanceTimer();
    const before = state.cards.map((card) => card.status).join('|');
    state = moveToThemeCard(state, index, options);
    if (before !== state.cards.map((card) => card.status).join('|')) emitState();
    render();
  ...
  const confirmThemes = () => {
    ...
    render();
    announce(`${selected.length} selected theme${selected.length === 1 ? '' : 's'} confirmed for this page session.`);
    emitState('themejourneyconfirmed');
  };

  const renderPreview = (card, direction) => {
    if (!card) {
      return `<div class="theme-card-preview is-empty" aria-hidden="true"><small>${direction}</small><strong>Summary</strong></div>`;
    }
  ...
  const renderIndex = () => state.cards.map((card, index) => `
    <button type="button" class="theme-index-button status-${card.status}${index === state.currentIndex ? ' is-current' : ''}"
      ...
      ${index === state.currentIndex ? 'aria-current="step"' : ''}>
      <span>${card.number}</span><small>${statusShort[card.status]}</small>
    </button>`).join('');
    ...
    <aside class="theme-selection-tray" aria-label="Selected theme summary">
      ...
      <div class="theme-tray-list">
        ${summary.selected.length
          ? summary.selected.map((card) => `<button type="button" data-edit-card="${card.number - 1}">${card.theme}</button>`).join('')
          : '<p>Selected cards will stay visible here.</p>'}
      </div>
    </aside>`;
    ...
    <article class="theme-flip-card status-${current.status}${state.isFlipped ? ' is-flipped' : ''}" aria-label="Card ${current.number} of ${state.cards.length}: ${current.theme}">
      <div class="theme-flip-inner">
        <button type="button" class="theme-card-face theme-card-front" data-flip-card
          aria-expanded="${state.isFlipped}" aria-hidden="${state.isFlipped}"
          tabindex="${state.isFlipped ? '-1' : '0'}"
          aria-label="Reveal card ${current.number}: ${current.theme}">
        ...
        <div class="theme-card-face theme-card-back" aria-hidden="${!state.isFlipped}" ${state.isFlipped ? '' : 'inert'}>
  ...
  const renderSummaryGroup = (title, cards, className, actionLabel = 'Edit choice') => `
    <section class="theme-summary-group ${className}">
      ...
      ${cards.length
        ? `<div>${cards.map((card) => `<article><span>${card.icon}</span><div><strong>${card.theme}</strong><p>${card.teaser}</p></div><button type="button" data-edit-card="${card.number - 1}">${actionLabel}</button></article>`).join('')}</div>`
        : '<p>None yet.</p>'}
    </section>`;
  ...
  const renderSummary = (summary) => `
    <section class="theme-final-summary" aria-labelledby="theme-summary-title">
      ...
      <p>Every choice can still be changed. Nothing is stored beyond this page session.</p>
      ...
      ${state.showingSummary ? renderSummary(summary) : `
        <div class="theme-decision-layout">
          ...
          <div class="theme-stage">
            ${renderCard(current)}
            <p class="theme-next-preview"><span>Up next:</span> ${current.nextCardName}</p>
