async function injectReferenceArtwork() {
  const modules = await Promise.all(
    Array.from({ length: 8 }, (_, index) => import(`./reference-art-lock-${index + 1}.js`)),
  );
  const template = document.createElement('template');
  template.innerHTML = modules.map((module) => module.default).join('');
  const sprite = template.content.firstElementChild;
  if (sprite) document.body.prepend(sprite);
}

function initialiseReferencePage() {
...
injectReferenceArtwork()
  .catch(() => {
    // Page structure and navigation remain usable if an illustration fails to load.
  })
  .finally(initialiseReferencePage);
